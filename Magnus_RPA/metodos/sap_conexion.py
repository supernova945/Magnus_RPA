...
def obtener_perfil_activo():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT alias, usuario_sap, es_productivo FROM perfiles WHERE es_activo = 1"
        )
        ...
        if perfil:
            return {
                "alias": perfil[0],
                "usuario_sap": perfil[1],
                "es_productivo": bool(perfil[2]),
            }
        return None
    except:
        return None
...
def _buscar_saplogon_dinamico():
    ...
    try:
        k = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\WOW6432Node\SAP\SAP Shared"
        )
...
def conectar_sap_autologin():
    ...
    if not perfil_activo:
        raise Exception(
            "No hay ningún perfil marcado como 'Activo'. Abra 'Credenciales SAP' y active la estrella en su perfil."
        )
    ...
    if not clave:
        raise Exception(
            f"No se encontró la clave guardada para el perfil '{perfil_activo['alias']}'."
        )
    ...
    nombre_conexion = (
        cfg["conexion_prd"] if perfil_activo["es_productivo"] else cfg["conexion_qas"]
    )
    ...
    time.sleep(1.5)
    try:
        sbar = session.findById(
            "wnd[0]/sbar/pane[0]", False
        )  # False evita excepción si no existe
        if sbar:
    ...
    try:
        ...
        for r in range(rows):
            try:
                ...
                marcado = (
                    "✅ SELECCIONADO"
                    if doc_clean in docs_seleccionados_clean
                    else "⬜ Ignorado"
                )

                fila_msg = (
                    f"   {r:<6} | {doc:<12} | {monto:<12} | {ref[:20]:<20} | {marcado}"
                )
                print_log(fila_msg)
            except:
                pass
...
def _seleccionar_sociedad_popup(session, dato_sociedad, print_log):
    base = "wnd[1]/usr/subSUBSCREEN_STEPLOOP:SAPLSPO5:0150/sub:SAPLSPO5:0150"
    sociedad = str(dato_sociedad).strip().upper()
    textos_detectados = []

    for idx in range(10):
        radio_id = f"{base}/radSPOPLI-SELFLAG[{idx},0]"
        try:
            radio = session.findById(radio_id, False)
        except:
            radio = None
        ...
        textos = []
        for attr in ("Text", "text", "Tooltip", "tooltip", "Name", "name"):
            try:
                valor = getattr(radio, attr, "")
                if valor:
                    textos.append(str(valor))
            except:
                pass

        for prefix in ("txtSPOPLI-TEXTLINE", "txtSPOPLI-VAROPTION", "lblSPOPLI-VAROPTION"):
            for col in range(0, 8):
                try:
                    ctrl = session.findById(f"{base}/{prefix}[{idx},{col}]", False)
                    if ctrl:
                        valor = getattr(ctrl, "Text", "") or getattr(ctrl, "text", "")
                        if valor:
                            textos.append(str(valor))
                except:
                    pass

        texto_fila = " ".join(textos).strip()
        if texto_fila:
            textos_detectados.append(f"{idx}: {texto_fila}")

        if sociedad and sociedad in texto_fila.upper():
            radio.selected = True
            print_log(f"   [SAP] Sociedad {sociedad} seleccionada en popup (fila {idx}).")
            return True

    # Compatibilidad con el comportamiento validado para Guatemala.
    indices_conocidos = {"GT03": 0, "GT09": 1}
    if sociedad in indices_conocidos:
        idx = indices_conocidos[sociedad]
        radio = session.findById(f"{base}/radSPOPLI-SELFLAG[{idx},0]", False)
        if radio:
            radio.selected = True
            print_log(f"   [SAP] Sociedad {sociedad} seleccionada por mapa conocido.")
            return True

    if textos_detectados:
        print_log("   [SAP] Opciones de sociedad detectadas: " + " | ".join(textos_detectados))
    return False
...
def abrir_caja_sap(session, dato_sociedad, print_log=lambda x: None):
    ...
    try:
        ...
        raise Exception(
            f"No se pudo iniciar la transacción /n/DBM/CASHDESK. Error: {e}"
        )
            ...
            if not popup_procesado:
                try:
                    ...
                    if session.findById(radio_id, False):
                        if not _seleccionar_sociedad_popup(
                            session, dato_sociedad, print_log
                        ):
                            raise Exception(
                                f"No se pudo seleccionar la sociedad {dato_sociedad} en el popup de SAP."
                            )
                        session.findById("wnd[1]/tbar[0]/btn[0]").press()
                        popup_procesado = True
                except Exception as e:
                    raise Exception(f"Error seleccionando sociedad: {e}")
...
if __name__ == "__main__":
    ...
    if not perfil:
        print(
            "  ❌ ERROR: No se encontró ningún perfil marcado como activo en perfiles.db"
        )
    ...
    if not clave:
        print(
            f"  ❌ ERROR: No se encontró la clave para el alias '{perfil['alias']}' en el Llavero de Windows."
        )
    ...
    print(
        "         Asegúrate de que SAP Logon esté cerrado para probar el auto-login completo."
    )
