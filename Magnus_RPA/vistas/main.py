...
# Importar módulos de PySide6
from formularios.ui_about import Ui_about

# Importar formularios compilados
...
class ContenedorPrincipal(QMainWindow):
    def __init__(self):
        ...
        ##Se carga el vista de menu principal, que contiene el menú lateral y el contenedor de páginas
        ...
        self.id_vista_bienvenida = self.contenedor_vistas.addWidget(
            self.vista_bienvenida
        )
        self.id_vista_dash = self.contenedor_vistas.addWidget(self.app_dashboard)
        self.id_vista_operaciones = self.contenedor_vistas.addWidget(
            self.app_operaciones.ventana.centralWidget()
            if hasattr(self.app_operaciones.ventana, "centralWidget")
            else self.app_operaciones.ventana
        )
        self.id_vista_usuarios = self.contenedor_vistas.addWidget(
            self.app_usuarios.ventana.centralWidget()
            if hasattr(self.app_usuarios.ventana, "centralWidget")
            else self.app_usuarios.ventana
        )
        self.id_vista_about = self.contenedor_vistas.addWidget(
            self.vista_about.centralWidget()
            if hasattr(self.vista_about, "centralWidget")
            else self.vista_about
        )
        ...
        if hasattr(self.ui, "btn_analitica"):
            self.ui.btn_analitica.clicked.connect(
                lambda: self.cambiar_pantalla(self.id_vista_dash)
            )

        if hasattr(self.ui, "btn_procesar_datos"):
            self.ui.btn_procesar_datos.clicked.connect(
                lambda: self.cambiar_pantalla(self.id_vista_operaciones)
            )

        if hasattr(self.ui, "btn_credenciales"):
            self.ui.btn_credenciales.clicked.connect(
                lambda: self.cambiar_pantalla(self.id_vista_usuarios)
            )

        if hasattr(self.ui, "btn_acerca_de"):
            self.ui.btn_acerca_de.clicked.connect(
                lambda: self.cambiar_pantalla(self.id_vista_about)
            )
