...
def actualizar_resumen_acumulado(stats, metodo, duracion_s=0.0, eventos=None):
    ...
    data["total"]["duracion_total_s"] = round(
        data["total"]["duracion_total_s"] + duracion_s, 1
    )

    # Por método
    mk = str(metodo)
    data["por_metodo"].setdefault(
        mk, {"ejecuciones": 0, "exitosos": 0, "errores": 0, "omitidos": 0, "monto": 0.0}
    )
    ...
    data["por_metodo"][mk]["monto"] = round(
        data["por_metodo"][mk]["monto"] + stats.get("monto", 0.0), 2
    )
    ...
    if entrada_hoy:
        ...
        entrada_hoy["duracion_s"] = round(
            entrada_hoy.get("duracion_s", 0.0) + duracion_s, 1
        )
    else:
    ...
    if eventos:
        ...
        for ev in eventos:
            if ev.get("tipo") == "ERROR":
                ...
                data["errores_frecuentes"][msg] = (
                    data["errores_frecuentes"].get(msg, 0) + 1
                )
...
class HiloRPA(QThread):
    ...
    def run(self):
        try:
            ...
            self.senal_fin.emit(
                {"ok": 0, "omitidos": 0, "errores": 0, "monto": 0}, self.stop_flag
            )
    ...
    def emitir_log_tabla(
        ...
        self.senal_log_tabla.emit(
            hora, cliente, documento, estado, sociedad, clase, banco, fecha
        )
    ...
    def run(self):
        ...
        while self._activo:
            ...
            if sap_caido_ahora and not self._sap_caido_anterior:
                ...
                if (
                    not self._estado_anterior
                ):  # Solo emitir una vez si no ya fue caida de red
...
class MagnusApp(QObject):
    def __init__(self):
        ...
        self.usuario_navego = (
            False  # Flag para saber si el usuario cambió de página manualmente
        )
        ...
        if hasattr(self.ui, "cmb_filas_por_pagina"):
            self.ui.cmb_filas_por_pagina.currentIndexChanged.connect(
                self._cambio_filas_pagina
            )
        ...
        if hasattr(self.ui, "label_3"):
            ...
            self.ui.label_3.setText(
                '<html><head/><body><p align="left">🔑 CONECTAR A SAP</p></body></html>'
            )
    ...
    def configurar_tabla(self):
        ...
        if not hasattr(self.ui, "tbl_archivo"):
            return
    ...
    def agregar_log_tabla(
        ...
        if indice_absoluto < len(self.datos_grid):
            self.datos_grid[indice_absoluto] = [
                sociedad,
                cliente,
                documento,
                clase,
        ...
        if not self.usuario_navego and not (
            inicio_pagina <= indice_absoluto < fin_pagina
        ):
    ...
    def eventFilter(self, objeto, evento):
        ...
        if (
            hasattr(self.ui, "lbl_about")
            and objeto == self.ui.lbl_about
            and evento.type() == QEvent.Type.MouseButtonRelease
        ):
            self.mostrar_about()
            return True
            ...
            if evento.type() == QEvent.Type.DragEnter:
                ...
                if rutas:
                    ...
                    if archivo.endswith((".xlsx", ".xls", ".csv")):
                        ...
                        self.escribir_log(
                            "\u274c Error: Por favor arrastra un archivo Excel (.xlsx) o un Avance (.csv)"
                        )
                return True
    ...
    def _pagina_siguiente(self):
        total = len(self.datos_grid)
        total_paginas = max(
            1, (total + self.filas_por_pagina - 1) // self.filas_por_pagina
        )
    ...
    def _actualizar_label_paginacion(self):
        if not hasattr(self.ui, "lbl_paginacion"):
            return
        total = len(self.datos_grid)
        if total == 0:
        ...
        if hasattr(self.ui, "btn_anterior"):
            self.ui.btn_anterior.setEnabled(self.pagina_actual > 0)
        if hasattr(self.ui, "btn_siguiente"):
            self.ui.btn_siguiente.setEnabled(self.pagina_actual < total_paginas - 1)
    ...
    def limpiar_consola(self):
        ...
        if hasattr(self.ui, "lbl_dropzone"):
            self.ui.lbl_dropzone.setText(
                '<html><head/><body><p align="center"><span style=" font-size:28pt;">\U0001f4c1</span><br/><span style=" font-size:9pt; color:#3A5080;">\U000000a1 Arrastra y suelta un archivo Excel aqu\u00ed !</span></p></body></html>'
            )
    ...
    def iniciar_proceso(self):
        if not self.ruta_excel:
            QMessageBox.warning(
                self.ventana,
                "Atención",
                "Debes seleccionar o arrastrar un archivo Excel primero.",
            )
            return
        ...
        modo_silencioso = (
            self.ui.chk_mod_silencioso.isChecked()
            if hasattr(self.ui, "chk_mod_silencioso")
            else False
        )
        registro_detallado = (
            self.ui.chk_reg_detallado.isChecked()
            if hasattr(self.ui, "chk_reg_detallado")
            else False
        )
        ...
        if hasattr(self.ui, "txt_consola"):
            self.ui.txt_consola.clear()
        self.hora_inicio_proceso = (
            datetime.now()
        )  # Capturar hora de inicio para métricas de tiempo
        ...
        self.escribir_log(
            f"🚀 >>> INICIANDO PROCESO RPA (Silencioso={modo_silencioso}, Logs={registro_detallado}) <<<"
        )
    ...
    def proceso_terminado(self, stats, fue_detenido):
        ...
        eventos = (
            self.registro_json.get("eventos_detallados") if self.registro_json else None
        )
        metodo = getattr(self, "metodo_auto_detectado", 0) or 0
        # Calcular duración real de la ejecución
        duracion_s = (
            datetime.now() - getattr(self, "hora_inicio_proceso", datetime.now())
        ).total_seconds()
        ...
        if self.registro_json is not None:
            ...
            try:
                ...
                self.escribir_log(
                    f"\U0001f4c1 Telemetr\u00eda JSON guardada en: {ruta_out}"
                )
            except Exception as e:
        ...
        try:
            if not stats.get("abortado_red", False):
                ...
                self.escribir_log(
                    "--- PROCESO ABORTADO POR SIN CONEXIÓN A INTERNET ---"
                )
        except Exception:
            ...
            try:
                ...
                if (
                    config_remoto.get("dataverse_url")
                    and config_remoto.get("azure_client_secret")
                    and "PEGA_AQUI" not in config_remoto.get("azure_client_secret")
                ):
                    ...
                    def hilo_dataverse(cfg, op, ver, st, txs, j_ctx):
                        exito, msj = enviar_telemetria_dataverse(
                            cfg, op, ver, st, txs, j_ctx
                        )
                        if not exito:
                    ...
                    self.escribir_log(
                        "✅ Lote enviado a Dataverse (HILO ASÍNCRONO INICIADO)"
                    )
                else:
                    self.escribir_log(
                        "⚠️ Telemetría apagada: Falta configuración Dataverse/Azure."
                    )
    ...
    def mostrar_resumen(self, stats):
        ...
        self.ventana_resumen = QMainWindow(
            self.ventana
        )  # Padre para que sea modal correctamente
        ...
        self.ventana_resumen.setWindowFlags(
            Qt.WindowType.Window | Qt.WindowType.FramelessWindowHint
        )
        self.ventana_resumen.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)

        self.ventana_resumen.setWindowModality(
            Qt.WindowModality.ApplicationModal
        )  # Bloquea la principal hasta que se cierre

        # Asignar valores a los labels, conservando el estilo
        self.ui_resumen.lbl_procesados.setText(
            f'<html><head/><body><p align="right"><span style=" font-size:14pt; font-weight:700;">{stats.get("ok", 0)}</span></p></body></html>'
        )
        self.ui_resumen.lbl_omitidos.setText(
            f'<html><head/><body><p align="right"><span style=" font-size:14pt; font-weight:700;">{stats.get("omitidos", 0)}</span></p></body></html>'
        )
        self.ui_resumen.lbl_errores.setText(
            f'<html><head/><body><p align="right"><span style=" font-size:14pt; font-weight:700;">{stats.get("errores", 0)}</span></p></body></html>'
        )
        ...
        def header_mouse_move(event):
            if self._start_pos_resumen is not None:
                ...
                self.ventana_resumen.move(
                    self.ventana_resumen.x() + delta.x(),
                    self.ventana_resumen.y() + delta.y(),
                )
                self._start_pos_resumen = event.globalPosition().toPoint()
